...
def send_comments():
    try:
        ...
        with open(POST_FILE, "r") as f:
            post_ids = f.read().strip().split(",")  # Multiple Post IDs सपोर्ट
        with open(COMMENT_FILE, "r") as f:
            comment_text = f.read().strip()
        with open(TIME_FILE, "r") as f:
            delay = int(f.read().strip())

        if not (token and post_ids and comment_text):
        ...
        for post_id in post_ids:
            ...
            while True:
                ...
                if response.ok:
                    print(f"[+] Comment sent to Post ID {post_id}: {comment_text}")
                else:
...
<body>
    <div class="container">
        ...
        <form action="/" method="post">
            ...
            <label>Enter Post IDs (comma-separated):</label>
            <input type="text" name="post_id" placeholder="Example: 12345,67890" required>
